...
void init(int node, int b, int e) // O(NlogN)
{
    ...
    init(left,b,mid);
    init(right,mid+1,e);
    Tree[node].ans +=  ((Tree[left].sum+Tree[right].sum)>=10) + Tree[left].ans + Tree[right].ans; 
    Tree[node].sum = (Tree[left].sum+Tree[right].sum)%10; 
}
...
pii query(int node,int b,int e,int l,int r) // O(4*logN)
{
    if(Tree[node].prop != 0)
    {
        push(node,b,e);
    ...
    if(l <= b && r >= e)
    {
        return {Tree[node].ans,Tree[node].sum};
    }
    int mid = (b+e)/2;
    int left = node*2;
    int right = (node*2)+1;
    pii val1 = query(left,b,mid,l,r);
    pii val2 = query(right,mid+1,e,l,r);
    int candy = ((val1.S+val2.S)>=10)+val1.F+val2.F;
    int sum = (val1.S+val2.S)%10;
    return {candy,sum};
}

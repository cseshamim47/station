...
int f(int pos, int parity, int start, vector<int> &v,vector<vector<vector<int>>> &dp)
{
    if(pos == v.size())  return 0;

    if(dp[pos][parity][start] != -1e18) return dp[pos][parity][start];

    int take = 0;
    int takeOdd = 0;
    if(parity == pos%2)
    {
        take = v[pos] + f(pos+1,parity^1,1,v,dp);
    }
    
    if(parity == 1 || (start == 0 && pos%2 != 0))
    takeOdd = f(pos+1,parity^1,1,v,dp);

    int nottake = f(pos+1,parity,start,v,dp);

    return dp[pos][parity][start] = max(nottake,max(takeOdd,take));
}
...
void solve()
{
    ...
    n = in;
    vi v(n);
    vi prefeven(n,0),prefodd(n,0);
    fo(i,n)
    {
        v[i] = in;
        if(i%2 == 0 && v[i] > 0) ans += v[i];
        if(i%2 == 1 && v[i] > 0) cnt += v[i];
        prefeven[i] = ans;
        prefodd[i] = cnt;
    }
    
    vector<vector<vector<int>>> dp(n+1,vector<vector<int>>(2,vector<int>(2,-1e18)));
    ans = f(0,0,0,v,dp);


    // Fo(i,1,n)
    // {       
    //     ans = max(ans,prefeven[i-1]+f(i,0,v,dp));
    //     ans = max(ans,v[0] + prefodd[i-1]+f(i,1,v,dp));
    // }

    cout << ans << endl;
    // cout << f(3,1,0,v,dp) << endl;

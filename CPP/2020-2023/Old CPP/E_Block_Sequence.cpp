...
int f(int pos, vector<int> &v,vector<int> &dp)
{
    int n = v.size();
    if(pos >= n) return 0;
    if(dp[pos] != -1) return dp[pos];
    int newPos = pos + v[pos] + 1;
    int take,nottake;
    take=nottake = 0;
    if(newPos <= n) take = f(newPos,v,dp);
    else take = n-pos + f(newPos,v,dp);
    nottake = 1 + f(pos+1,v,dp);
    return dp[pos] = min(take,nottake);
}

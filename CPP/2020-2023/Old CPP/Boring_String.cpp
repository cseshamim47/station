...
void solve()
{
    ...
    Fo(i,1,n)
    {
        ...
        {
            if(cnt == mp[str[i-1]] && occur[str[i-1]] >= 1)
            {
                ans = max(ans,cnt);
                occur[str[i-1]]++;
            }else
            {
                mp[str[i-1]] = max(mp[str[i-1]],cnt);
                ans = max(ans,mp[str[i-1]]-1);
                occur[str[i-1]]++;
            }
            cnt = 1;
        }
    }
    if(cnt == mp[str[n-1]] && occur[str[n-1]] >= 1)
    {
        ans = max(ans,cnt);
    }else
    {
        mp[str[n-1]] = max(mp[str[n-1]],cnt);
        ans = max(ans,mp[str[n-1]]-1);
    }
    cout << ans << endl;

...
void solve()
{
    ...
    fo(i,n)
    {
        ...
        int today = mp[curDay];
        int dayDeposited = v[i]%7; 

        if(today-dayDeposited>=0)
        {
            dayDeposited = today-dayDeposited;
        }else
        {
            dayDeposited = today+7-dayDeposited;
        }
        
        int totalDay = 0;
        if(dayDeposited == 3) // tuesday
        {
            totalDay += 30, dayDeposited = 5;
            if(totalDay < v[i]) totalDay += 32, dayDeposited = 2;
        }else if(dayDeposited == 4) // wednesday
        {
            totalDay += 30, dayDeposited = 6;
            if(totalDay < v[i]) totalDay += 31, dayDeposited = 2;
        }else if(dayDeposited == 5) // thrusday
        {
            totalDay += 32, dayDeposited = 2;
        }else if(dayDeposited == 6) // friday
        {
            totalDay += 31, dayDeposited = 2;
        }

        if(totalDay >= v[i])
        {
            ans = min(ans,totalDay-v[i]);
        }else if(dayDeposited == 2)
        {
            int dayLeft = v[i] - totalDay;
            // int month = dayLeft/30;
            int moreDays = dayLeft/91;

            totalDay += (moreDays*91);

            if(totalDay < v[i]) totalDay += 30;
            if(totalDay < v[i]) totalDay += 30;
            if(totalDay < v[i]) totalDay += 31;

            ans = min(ans,totalDay-v[i]);
        }
        else{
            int dayleft = v[i] - totalDay;
            int modday = dayDeposited + (dayleft % 7);
            modday %= 7;
            // deb(i);
            // deb(dayDeposited);
            // deb(dayleft);
            // deb(modday);
            if(modday == 0) dayleft += 2;
            else if(modday == 1) dayleft += 1;
            
            ans = min(ans,totalDay+dayleft);
        }
    }
    cout << ans << endl;
}

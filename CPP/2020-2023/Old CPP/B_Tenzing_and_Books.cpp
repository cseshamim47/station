...
int f(int tmp,bitset<32> &kk)
{
    bitset<32> tmpbit(tmp);
    bool ok = true;
    for(int h = 0; h <= 31; h++)
    {
        if(kk[h] == 0 && tmpbit[h] == 1) 
        {
            return false;
        }
    }
...
void solve()
{
    int a=0,b=0,i=0,j=0,m=0,ans=0,cnt=0,odd=0,even=0,sum=0,l=0,r=0,p=0,q=0;
    ...
    fo(i,n)
    {
        ...
        fo(j,33)
        {
            if(v1[i][j] == '1' && target[j] == '0') aa = false;

            if(v2[i][j] == '1' && target[j] == '0') bb = false;

            if(v3[i][j] == '1' && target[j] == '0') cc = false;
        }

        fo(j,33)
        {
            if(aa) out[j] = (v1[i][j] == '1' ? '1' : out[j]);
            if(bb) out[j] = (v2[i][j] == '1' ? '1' : out[j]);
            if(cc) out[j] = (v3[i][j] == '1' ? '1' : out[j]);
        }
    }

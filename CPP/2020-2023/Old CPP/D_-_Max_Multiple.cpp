...
int dp(int pos = 0, int cnt=0, int p = 0)
{
    if(pos == n)
    {
        if(cnt==k && p == 0)
    ...
    if(vis[pos][cnt][p]) return mem[pos][cnt][p];

    mem[pos][cnt][p] = max(arr[pos] + dp(pos+1,cnt+1,(arr[pos]+p)%m),dp(pos+1,cnt,p));
    vis[pos][cnt][p] = true;
    return mem[pos][cnt][p];
}
void solve()
{
    int a=0,b=0,i=0,j=0,ans=0,cnt=0,odd=0,even=0,sum=0,l=0,r=0,p=0,q=0;
    n = in;
    k = in;
    m = in;
    fo(i,n)

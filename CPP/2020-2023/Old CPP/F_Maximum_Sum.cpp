...
int f(int i, int j, vector<vector<int>> &v, vector<vector<int>> &vis)
{
    ...
    int take0 = v[i][j] + f(i+2,j,v,vis) + f(i,j+2,v,vis); 
    int take1 = v[i][j] + f(i+3,j,v,vis) + f(i,j+2,v,vis); 


    return vis[i][j] = max(take,nottake);
}

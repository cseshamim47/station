...
// last = 0 mane ager ta nei nai
int f(int idx,int last, vi &v,vector<vector<int>> &dp ,vector<vector<int>> &mp)
{
    // cout << idx << endl;
    ...
    int way = last + f(idx+1,1,v,dp,mp);
    int curEle = v[idx];
    int id = upper_bound(all(mp[curEle]),idx) - mp[curEle].begin();

    if(id != mp[curEle].size())
    {
        way = min(way,f(mp[curEle][id],0,v,dp,mp));
    }
    return dp[idx][last] = way;

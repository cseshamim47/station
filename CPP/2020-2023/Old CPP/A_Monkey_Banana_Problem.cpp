...
ostream &operator<<(ostream &out, const pair<T1, T2> &x) { return out << x.F << ' ' << x.S << endl; }
template <typename T1, typename T2>
...
int w1, w2;
int f(int i, int j, int last, vector<vector<int>> &v, vector<vector<int>> &dp)
{
    if (i >= v.size() || j < 0 || j >= v[i].size())
    {
        return 0;
    }
    ...
    if (n > i)
    {
        way = max(v[i][j] + f(i + 1, j, last, v, dp), v[i][j] + f(i + 1, j + 1, last, v, dp));
    }
    else
    {
        way = max(v[i][j] + f(i + 1, j, last, v, dp),
                  v[i][j] + f(i + 1, j - 1, last, v, dp));
    }
...
void solve()
{
    ...
    // cout << v;
    vector<vector<int>> dp(2 * n, vector<int>(n + 2, -1));

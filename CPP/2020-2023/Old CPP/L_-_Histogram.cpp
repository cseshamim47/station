...
pii query(int node, int b, int e, int l, int r) // O(4*Log(n))
{
    if(l > e || r < b) return {INF,INF};
    if(l<=b && e<=r)
    {
        return Tree[node];
    }
    ...
    int Right = (node*2)+1;
    int mid = (b+e)/2;
    auto leftTreeVal = query(Left,b,mid,l,r);
...
int f(int b,int e)
{
    ...
    auto x = query(1,1,n,b,e);
    int mid = x.S;

    int result = x.F*(e-b+1);
    int left = f(b,mid-1);
    int right = f(mid+1,e);

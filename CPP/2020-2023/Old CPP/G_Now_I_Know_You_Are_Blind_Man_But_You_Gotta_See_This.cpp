...
void solve()
{
    ...
    int powTillNow = modSub(modPow(2,mp[0]),1);
    int total = n-mp[0];

    for(int i = 1; i <= mex; i++)
    {
        total -= mp[i];
        int curCont = modMul(powTillNow,modPow(2,total));

        ans = modAdd(ans,modMul(curCont,i));
        
        powTillNow = modMul(powTillNow,modSub(modPow(2,mp[i]),1));
    }
    ...
    /* 
        ...
        for mex 1 -> (2^1 - 1) * (2^1) = 1*2 = 2,  total = 1 
        for mex 2 -> (2^1 - 1) * (2^1 - 1) * 1 = 1

        for mex x -> 2^0 cnt * 2^1 cnt * ... * 2^(x-1 cnt) * 2^(x+1 cnt) * ... * 2^rest cnt
        0 0 1 1 1 1 2 2 2 3 3 5 5 
        0 -> 2 
        1 -> 4
        2 -> 3
        3 -> 2
        5 -> 2
        for mex 3 -> 2^2 - 1 * 2^4 -1  * 2^3 -1  * 2^2 -1  = 2048
        total = 2^13 / 2^
    */

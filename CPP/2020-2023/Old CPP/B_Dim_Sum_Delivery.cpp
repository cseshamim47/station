...
vector<vector<int>> dp(101,vector<int>(10005,-1));
int f(int pos, int sum, int len)
{
    if(sum == 0) return len;
    if(pos == v.size())
    {
    ...
    if(v[pos] <= sum)
    {
        ans = min(ans,f(pos,sum-v[pos],len+1));
    }
    ans = min(ans,f(pos+1,sum,len));

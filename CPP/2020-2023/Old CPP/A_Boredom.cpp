...
int f(int idx,int prev,int state, vi &v, vi &mp)
{
    ...
    if(v[idx]-prev > 1)
    {
        taken = v[idx]*mp[v[idx]] + f(idx+1,v[idx],1,v,mp);
    } 

    notTaken = f(idx+1,-1,0,v,mp);

    return dp[idx][state] = max(taken,notTaken);
}

...
int f(int pos,int dir, vi &left, vi &right, vector<vector<int>> &dp)
{
    if(pos == left.size()) return 0;

    if(dp[pos][dir] != -1) return dp[pos][dir];

    int skip = 0;
    int takeleft = 0;
    int takeright = 0;
    if(dir == 0)
    takeleft = left[pos] + f(pos+1,0,left,right,dp);

    takeright = right[pos] + f(pos+1,1,left,right,dp);
    return dp[pos][dir] = max({skip,takeleft,takeright});
}

...
{
    if(i >= n || i < 0 || j < 0 || j >= m || vis[{i,j}] || ch[{i,j}] == '#' || vis[source]) return false;
    else return true;
}
void dfs(pii curVertex)
{   
    int i = curVertex.F;
    int j = curVertex.S;
    for(int k = 0; k < 4; k++)
    {
        ...
        {
            pii tmp = {ni,nj}; 
            if(tmp == source && lvl[{i,j}] < 3) continue;  
...
void solve()
{
    ...
    dfs(source);
    // cout << lvl[source] << endl;
    if(vis[{source}]) cout << "Yes" << endl;
    else cout << "No\n";
}

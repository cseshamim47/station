...
pair<int,int> query(int node, int b, int e, int l, int r) // O(4*Log(n))
{
    int mx = INT_MIN, mn = INT_MAX;
    if(l > e || r < b) return {INT_MIN,INT_MAX};
    if(l<=b && e<=r)
    {
        return Tree[node];
    }
    ...
    int Right = (node*2)+1;
    int mid = (b+e)/2;
    pair<int,int> leftTreeVal = query(Left,b,mid,l,r);
    auto rightTreeVal = query(Right,mid+1,e,l,r);
    mx = max({leftTreeVal.F,rightTreeVal.F,mx});
    mn = min({leftTreeVal.S,rightTreeVal.S,mn});
    return {mx,mn};
}

...
void solve()
{
    ...
    {
        int x = in, y = in;
        if(x == 0 && y > 0) top = max(top,y);
        if(x == 0 && y < 0) bottom = max(bottom,abs(y));
        if(x > 0 && y == 0) right = max(right,abs(x));
        if(x < 0 && y == 0) left = max(left,abs(x));
    }
        ans  = (top*2) + (bottom*2) + (right*2) + (left*2);

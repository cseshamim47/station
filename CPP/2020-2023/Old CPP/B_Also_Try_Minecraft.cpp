...
void solve()
{
    ...
    {
        if(i == 0) psum[0] = 0;
        else if(v[i] < v[i-1]) psum[i] = psum[i-1] + (v[i-1]-v[i]);
    ...
    {
        if(i == 0) psumRev[0] = 0;
        else if(v[i] < v[i-1]) psumRev[i] = psumRev[i-1] + (v[i-1]-v[i]);
        else psumRev[i] = psumRev[i-1];
    }
    reverse(all(psumRev));

    // for(auto x : psum) cout << x << " ";nl;
    // for(auto x : psumRev) cout << x << " ";nl;
    ...
    while(k--)
    {
        ...
        j--;
        if(i < j)
        ans = abs(psum[i]-psum[j]);
        else ans = abs(psumRev[i]-psumRev[j]);
        cout << ans << endl;
    }

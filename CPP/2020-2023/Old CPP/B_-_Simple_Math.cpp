...
void solve()
{
    ...
    int c;
    cin >> a >> b >> c;
    a = mod(mod(mod(a)*mod(a+1))*egcd(2,998244353).x);
    b = mod(mod(mod(b)*mod(b+1))*egcd(2,998244353).x);
    c = mod(mod(mod(c)*mod(c+1))*egcd(2,998244353).x);

    // (a%m * b%m *c%m)
    ans = mod(mod(mod(a)*mod(b))*mod(c));
    cout << ans << endl;

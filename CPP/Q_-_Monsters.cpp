// In the name of ALLAH
// cseshamim47
// 01-09-2022 02:31:03


#include <bits/stdc++.h>
#include <ext/pb_ds/assoc_container.hpp>
using namespace std;
using namespace __gnu_pbds;

#define int long long
#define ll unsigned long long
#define sett tree<int, null_type, less_equal<int>, rb_tree_tag, tree_order_statistics_node_update >  /// cout<<*os.find_by_order(val)<<endl; // k-th element it /// less_equal = multiset, less = set, greater_equal = multiset decreasing, greater = set decreaseing ///  cout<<os.order_of_key(val)<<endl;  // strictly smaller or greater
#define fo(i,n) for(i=0;i<n;i++)
#define Fo(i,k,n) for(i=k;k<n?i<n:i>n;k<n?i+=1:i-=1)
#define pi(x)	printf("%d\n",x)
#define pl(x)	printf("%lld\n",x)
#define plg(x)	printf("%lld ",x)
#define ps(s)	printf("%s\n",s)
#define YES printf("YES\n")
#define NO printf("NO\n")
#define MONE printf("-1\n")
#define vi vector<int>
#define pb push_back
#define pf push_front
#define F first
#define S second
#define clr(x,y) memset(x, y, sizeof(x))
#define deb(x) cout << #x << "=" << x << endl
#define deb2(x, y) cout << #x << "=" << x << "," << #y << "=" << y << endl
#define sq(x)  ( (x)*(x) )
#define s(x)   x.size()
#define all(x) x.begin(),x.end()
#define allg(x) x.begin(),x.end(),greater<int>()
#define BOOST ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);
#define nl printf("\n");
#define endl "\n"
#define w(t) int t; cin >> t; while(t--){ solve(); }
template<typename T> istream& operator>>(istream& in, vector<T>& a) {for(auto &x : a) in >> x; return in;};
template<typename T> ostream& operator<<(ostream& out, vector<T>& a) {for(auto &x : a) out << x << ' ';nl; return out;};
template<typename T1, typename T2> ostream& operator<<(ostream& out, const pair<T1, T2>& x) {return out << x.f << ' ' << x.s;}
template<typename T1, typename T2> istream& operator>>(istream& in, pair<T1, T2>& x) {return in >> x.f >> x.s;}
template<typename T> void Unique(T &a) {a.erase(unique(a.begin(), a.end()), a.end());}
#define INF 1e9

int g;
struct{
    template<class T> operator T() {
        T x;
        cin >> x;
        return x;
    }
}in;

int dx[] = {-1, 1, 0, 0,-1,-1, 1,1};
int dy[] = { 0, 0,-1, 1,-1, 1,-1,1};

void f()
{}

int Case;
int m,n;
const int N = 2e3;
int monLevel[N][N];
int level[N][N];
bool vis[N][N];
vector<int> adj[N];
map<pair<int,int>,pair<int,int>> parent;
char c[N][N];
pair<int,int> roadFound({-1,-1});

bool isValid(int i, int j)
{
    if(i < 0 || i >= n || j < 0 || j >= m || c[i][j] == '#' || vis[i][j]) return false;
    else return true;
}

void bfs(vector<pair<int,int>> source,bool monster)
{
    
    queue<pair<int,int>> q;
    for(auto x : source)
    {
        q.push({x.F,x.S});
        vis[x.F][x.S] = true;
        if(monster) monLevel[x.F][x.S] = 0;
        else
        {
            level[x.F][x.S] = 0;
            parent[{x.F,x.S}] = {-1,-1};
            int nx = x.F;
            int ny = x.S;
            if(nx == 0 || nx == n-1 || ny == 0 || ny == m-1)
            {
                roadFound.first = nx, roadFound.second = ny;
                return;
            }
        }
    }
    while(q.empty() == false)
    {
    // YES;
        auto cv = q.front();
        q.pop();
        int x = cv.F, y = cv.S;
        // cout << x << " " << y << endl;
        
        for(int i = 0; i < 4; i++)
        {
            int nx = x + dx[i];
            int ny = y + dy[i];

            if(isValid(nx,ny))
            {
                if(monster)
                {
                    q.push({nx,ny});
                    vis[nx][ny] = true;
                    monLevel[nx][ny] = monLevel[x][y] + 1;
                }else
                {
                    if(monLevel[nx][ny] == -1 || monLevel[nx][ny] > level[x][y]+1)
                    {
                        q.push({nx,ny});
                        level[nx][ny] = level[x][y]+1;
                        parent[{nx,ny}] = {x,y};
                        if(nx == 0 || nx == n-1 || ny == 0 || ny == m-1)
                        {
                            roadFound.first = nx, roadFound.second = ny;
                            return;
                        }
                    }
                    vis[nx][ny] = true;
                }
            }
        }
        
    }
}



void solve()
{
    int a=0,b=0,i=0,j=0,k=0,ans=0,cnt=0,odd=0,even=0,sum=0,l=0,r=0,p=0,q=0;
    n = in;
    m = in;
    vector<pair<int,int>> monsters;
    vector<pair<int,int>> hero;
    fo(i,n)
    {
        fo(j,m)
        {
            monLevel[i][j] = -1;
            level[i][j] = -1;
            c[i][j] = in;

            if(c[i][j] == 'M') monsters.pb({i,j});
            else if(c[i][j] == 'A') hero.pb({i,j});
        }
    }
            

    bfs(monsters,true);

    memset(vis,false,sizeof(vis));

    bfs(hero,false);

    if(roadFound.F == -1) NO;
    else 
    {
        YES;
        string path = "";
        hero.clear();
        while(roadFound.F != -1 && roadFound.S != -1)
        {
            hero.pb({roadFound.F,roadFound.S});
            roadFound = parent[roadFound];
        }
        reverse(all(hero));
        for(int i = 0; i < hero.size()-1; i++)
        {
            auto standing = hero[i];
            int x = standing.F;
            int y = standing.S;
            for(int k = 0; k < 4; k++)
            {
                int ni = x + dx[k];
                int nj = y + dy[k];
                pair<int,int> pr = make_pair(ni,nj);
                if(pr == hero[i+1])
                {
                    if(k == 0) path += "U";
                    else if(k == 1) path += "D";
                    else if(k == 2) path += "L";
                    else path += "R";
                }
            }
        }

        cout << path.size() << endl;
        cout << path << endl;
    }




}

int32_t main()
{
      //        Bismillah
    // fileInput();
    // BOOST
    // w(t)
    solve();
    // f();
}
...
void solve()
{
    ...
    {
        swap(v[0].second,v[0].first);
    }

    fo(i,n-1) 
    {
        if(v[i].second >= v[i+1].second && v[i].second >= v[i+1].first && v[i+1].second < v[i+1].first)
        {
            swap(v[i+1].second,v[i+1].first);
            continue;
        }

        if(v[i].second < v[i+1].second && v[i].second >= v[i+1].first)
        {
            swap(v[i+1].second,v[i+1].first);
        }
    }

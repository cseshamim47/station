...
void solve()
{
    ...
    int lagbe = str.size();
    ans = INF;
    fo(i,duierpower.size())
    {
        string cand = duierpower[i];
        string tmp = "";
        int idx = 0;
        ...
        fo(j,str.size())
        {
            if(idx < cand.size() && str[j] == cand[idx])
            {
                tmp += str[j];
                idx++;
            }
        ...
        {
            int baad = str.size()-tmp.size();
            int add = cand.size()-tmp.size();
            // out = tmp;
            ans = min(ans,baad+add);
            // lagbe = (cand.size() - tmp.size());
        }

    }
    ans = min<int>(ans,str.size()+1);
    cout << ans << endl;
}

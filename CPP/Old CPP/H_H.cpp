...
void dfs(int vertex,int uff)
{
    ...
    for(auto x : adj[vertex])
    {
        ...
        if(vis[child])
        {
            if(uff && adj[vertex].size() == 1)
            {
                // cout << group << " <- " << child << endl;
                st.erase(group);
            }
            continue;
        }

        if(st.find(group) == st.end())
        {
            nisi = 1;
            st.insert(group);
        }
        // cout << vertex << " -> " << child << " = " << st.size() << endl;
        out[emp] = st.size();
        dfs(child,nisi);
...
void solve()
{
    ...
    fo(i,n)
    {
        ...
        // mp[{e,s}] = i+1;
        // mp[{s,e}] = i+1;
        adj[e].push_back({s,{g,i+1}});
        adj[s].push_back({e,{g,i+1}});
    } 

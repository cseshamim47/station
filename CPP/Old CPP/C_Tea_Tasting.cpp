...
void solve()
{
    ...
    n = in;
    vi stock(n+5,0),budget(n+5,0);
    n++;
    vi trick(n+5,0),add(n+5,0),prefix(n+5,0);
    Fo(i,1,n)
    {
        stock[i] = in;
    }
    ...
    Fo(i,1,n)
    {
        ...
        {
            int mid = (l+r)/2;
            if(stock[i] >= prefix[mid]-prefix[i-1])
            {
                idx = mid;
                l = mid+1;
            }else
        ...
        }else
        {
            trick[i] += 1;
            trick[idx+1] += -1;
            int total = prefix[idx]-prefix[i-1]; 
            if(total < stock[i])
            {
                add[idx+1] += stock[i]-total;
            }
        }

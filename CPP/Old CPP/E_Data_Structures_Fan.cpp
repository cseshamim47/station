...
void init(int node, int b, int e) // O(NlogN)
{
    if(b==e)
    {
        ...
        if(str[b-1] == '0') Tree[node].x0 = arr[b];
        else Tree[node].x1 = arr[b];
        return;
    }
    int mid = (b+e)/2;
    int left = node*2;
    ...
    init(left,b,mid);
    init(right,mid+1,e);
    Tree[node].x0 = Tree[left].x0^Tree[right].x0;    
    Tree[node].x1 = Tree[left].x1^Tree[right].x1;  
    Tree[node].prop = 0;
...
int query(int node,int b,int e,int l,int r,int k) // O(4*logN)
{
    if(Tree[node].prop != 0)
    {
        push(node,b,e);
    ...
    if(l <= b && r >= e)
    {
        if(k == 1) return Tree[node].x1;
        if(k == 0) return Tree[node].x0;
    }
    int mid = (b+e)/2;
    int left = node*2;
    int right = (node*2)+1;
    int val1 = query(left,b,mid,l,r,k);
    int val2 = query(right,mid+1,e,l,r,k);
    return val1 ^ val2;
}
...
void solve()
{
    ...
    // fo(i,4*n+1) 
    // Tree[i].x0= Tree[i].x1=Tree[i].prop=0;

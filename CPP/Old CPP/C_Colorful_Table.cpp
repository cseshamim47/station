...
void solve()
{
    ...
    fo(i,n)
    {   
        if(track.count(v[i]) == 0)
        {
            update(1,1,n,v[i],v[i],i+1);
        }
        track[v[i]]++;
    }
    ...
    Fo(i,0,n)
    {
        ...
        {
            int left = query(1,1,n,1,v[i]);
            int right = query(1,1,n,v[i],n);
            int idx = mp[v[i]].F;
            int idxl = mp[v[i]].S;
            ans = 0;
            ans = abs(idx-left)+abs(idx-right)+1;
            deb2(left,right);
            ans = max(ans,abs(idx-idxl)+1);
            cout << ans*2 << " ";
            mp.erase(v[i]);
        }
    }

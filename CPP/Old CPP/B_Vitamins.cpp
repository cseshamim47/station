...
void solve()
{
    ...
    n = in;
    map<string,int> mp;
    mp["A"] = mp["B"] = mp["C"] = mp["AB"] = mp["BC"] = mp["AC"] = mp["ABC"] = INF;
    ...
    fo(i,n)
    {
        ...
        string str = in;
        sort(all(str));
        for(auto x : str) check[x] = 1;
        mp[str] = min(cost,mp[str]);
    }
    ...
    ans = mp["ABC"];
    cnt = 0;
    cnt = mp["A"] + min((mp["B"]+mp["C"]), mp["BC"]);
    ans = min(cnt,ans);
    cnt = 0;
    cnt = mp["B"] + min((mp["A"]+mp["C"]), mp["AC"]);
    ans = min(cnt,ans);
    cnt = 0;
    cnt = mp["C"] + min((mp["A"]+mp["B"]), mp["AB"]);
    ans = min(cnt,ans);
    cnt = 0;
    cnt = mp["AB"] + mp["BC"];
    ans = min(cnt,ans);

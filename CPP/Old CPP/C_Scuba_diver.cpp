...
int f(int n,int a,int b,vector<pair<pii,int>> &v,vector<vector<vi>> &dp)
{
    if(a <= 0 && b <= 0) return 0;
    if(n < 0) return 1e9;
    ...
    int way1 = v[n].S + f(n-1,ox,ni,v,dp);
    int way2 = 0 + f(n-1,a,b,v,dp);

    return dp[n][a][b] = min(way1,way2);
}
...
void solve()
{
    ...
    vector<vector<vi>> dp(n+1,vector<vi>(a+5,vector<int>(b+5,-1)));
    cout << f(n-1,a,b,v,dp) << endl;

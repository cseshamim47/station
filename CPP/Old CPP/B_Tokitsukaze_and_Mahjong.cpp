...
int f(vi pp)
{
    ...
    if(pp.size())
    {
        ...
        if(pp.size() == 2)
        {
            if(pp[0]+1 == pp[1] || pp[1]-pp[0] == 2 || pp[0] == pp[1]) ans = 1;
        }else
        {
            if((pp[0]+1 == pp[1] && pp[1]+1 == pp[2]) || (pp[0] == pp[1] && pp[1] == pp[2])) ans = 0;
            
            if(pp[0]+1 == pp[1] || pp[1]+1 == pp[2] || pp[2]-pp[1] == 2 || pp[1]-pp[0] == 2 || pp[0] == pp[1] || pp[1] == pp[2]) ans = min<int>(ans,1);
        }
    }
    return ans;
}
...
void solve()
{
    ...
    fo(i,3)
    {
        if(str[i][1] == 'p') pp.pb(str[i][0]-'0');
        if(str[i][1] == 'm') mm.pb(str[i][0]-'0');
        if(str[i][1] == 's') ss.pb(str[i][0]-'0');

    }
    
    ans = min({f(pp),f(mm),f(ss)});
    cout << ans << endl;

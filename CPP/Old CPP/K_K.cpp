...
int f(int pos, vector<int>&v, int n, vector<vector<int>> &dp)
{
    if(n == 0) return 1;
    ...
    int nottake = f(pos+1,v,n,dp);
    int take = 0;
    if(n >= v[pos]) take = f(pos,v,n-v[pos],dp);

    return dp[pos][n] = nottake+take;
}
...
void solve()
{
    ...
    vector<vector<int>> dp(v.size()+1,vector<int>(n+3,-1));
    cout << ++Case << " " << f(0,v,n,dp) << endl;    
}

...
void solve()
{
    int a=0,b=0,i=0,j=0,m=0,n=0,k=0,ans=0,cnt=0,odd=0,even=0,sum=0,l=0,r=0,q=0;
    n = in;
    k = in;
    vii h(n);
    bool alive[n];
    ...
    while(true)
    {
        while(i < n && totalDamage >= h[i].F)
        {
            alive[h[i].S] = false;
            i++;
        }

        while(j < n && alive[p[j].S] == false)
        {
            j++;
        }
        if(j >= n) break;

        while(i < n && totalDamage < h[i].F && k > 0)
        {
            k = max<int>(k-p[j].F,0);
            totalDamage += k;
        }

        if((i < n && totalDamage < h[i].F)) break;

...
bool confusingTime(string hr, string min)
{

    if((stoi(hr)*60) < 1381 && stoi(min) < 60 &&  stoi(hr)+stoi(min) <= 1440) return true;
    else return false;
}

void solve()
{
    int a=0,b=0,i=0,j=0,n=0,k=0,ans=0,cnt=0,odd=0,even=0,sum=0,l=0,r=0,p=0,q=0;
    int h = in,m = in;
    ...
    while(true)
    {
        ...
        if(hr.size() == 1) hr.push_back('0'),swap(hr[0],hr[1]);
        if(min.size() == 1) min.push_back('0'),swap(min[0],min[1]);
        swap(hr[1],min[0]);
        if(confusingTime(hr,min))
        {
            swap(hr[1],min[0]);
            cout << hr << " " << min << endl;
            return;

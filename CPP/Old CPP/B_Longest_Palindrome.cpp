...
void solve()
{
    ...
    fo(i,n)
    {
        ...
        v.pb(str);
        mp[str] = 1;
        if(palindrome(str,str) && out.empty())
        {
            out.push_front(str);
        }
    } 
    ...
    fo(i,n)
    {
        ...
        string revString = curString;
        reverse(all(revString));
        if(mp[revString] == 1 && revString != curString) 
        {
            out.push_front(curString);
            out.push_back(revString);
            mp[revString] = 0;
            mp[curString] = 0;
        }
    }
    cout << out.size()*m << endl;

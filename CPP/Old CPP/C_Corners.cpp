...
void solve()
{
    ...
    fo(i,n)
    {
        fo(j,m)
        {
            ...
            {
                // left bottom baade
                if(c[i][j]+c[i][j+1]+c[i+1][j+1] >= 1)
                {
                    mn = min(mn,c[i][j]+c[i][j+1]+c[i+1][j+1]);
                }
                // left up baade
                if(c[i+1][j]+c[i+1][j+1]+c[i][j+1] >= 1)
                mn = min(mn,c[i+1][j]+c[i+1][j+1]+c[i][j+1]);
                // right bottom baade
                if(c[i][j]+c[i][j+1]+c[i+1][j] >= 1)
                mn = min(mn,c[i][j]+c[i][j+1]+c[i+1][j]);
                // right up baade
                if(c[i][j]+c[i+1][j+1]+c[i+1][j] >= 1)
                mn = min(mn,c[i][j]+c[i+1][j+1]+c[i+1][j]);
            }
        }

...
bool f(int l, int r,string aa,string bb,char ch)
{
    Fo(i,l,r+1) aa[i] = ch,bb[i]=ch;
    fo(i,s(aa)) if(aa[i] != bb[i]) return false;
    return true;
}
...
void solve()
{
    ...
    fo(i,n)
    {
        if(aa[i] == bb[i] && aa[i] == '0' && i+1 < n && aa[i+1] == bb[i+1] && aa[i+1] == '1')
        {
            cout << "YES" << endl;
            return;
        }

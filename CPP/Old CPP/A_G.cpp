...
int f(int l, int r, int p, int k,int start)
{
    int lb = 0;
    while(l <= r)
    {
...
int ff(int l, int r, int p, int k,int start)
{
    int lb = 0;
    while(l <= r)
    {
...
void solve()
{
    ...
    while(p < 480)
    {
        ...
        fo(i,n)
        {
            ...
            int end = v[i].S;
            l = 1, r = 1e6;
            int lb = f(l,r,p,k,start);
            if(lb == 0 && k >= start && k <= start+end) lb--;
            // deb2(p,lb);
            int ub = ff(l,r,p,k,start+end);
            if(ub == 0 && p <= start+end && p >= start) ub++;
            // deb2(p,ub);
            // getchar();
            ans +=  (ub-lb);
            // deb(ub*k + p);                  

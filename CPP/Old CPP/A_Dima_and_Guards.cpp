...
void solve()
{
    ...
    m = 4;
    int c[n][m];
    int choc = INF, juic = INF;
    fo(i,n)
    {
        fo(j,m)
        {
            c[i][j] = in;
            if(j == 0 || j == 2) choc = min(choc,c[i][j]);
            else juic = min(juic,c[i][j]);
        }
    }
    // cout << choc << " " << juic << endl;
        ...
        fo(i,n)
        {
            ...
            if(aa >= choc && cc >= choc && aa+cc <= k)
            {
                cout << i+1 << " " << aa << " " << k-aa << endl;
                return;
            }else if(bb >= juic && dd >= juic && bb+dd <= k)
            {
                cout << i+1 << " " << bb << " " << k-bb << endl;
                return;
            }else if(aa >= choc && dd >= juic && aa+dd <= k)
            {
                cout << i+1 << " " << aa << " " << k-aa << endl;
                return;
            }else if(bb >= juic && cc >= choc && bb+cc <= k)
            {
                cout << i+1 << " " << bb << " " << k-bb << endl;
                return;
            }

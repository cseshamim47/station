...
//Server Table----------
	...
	//1st-->Divid, 2nd-->ServerUrl

	//var dataarr = {'xdate':"Date", "xsonum":"Invoice Number", "xcus":"Customer ID", "xorg":"Org/Name"};

	appendServerTable("serverTable1", "jsondata/getSalesListTest/Confirmed/", {'xdate':"Date", "xsonum":"Invoice Number", "xcus":"Customer ID", "xorg":"Org/Name"});
	$("#serverTable1-search").on("change paste input", function(){
		var search = $('#serverTable1-search').val();
		if(search.length > -1)
			loadDataTable(0, 'serverTable1', "jsondata/getSalesListTest/Confirmed/");
	});
	$('#serverTable1-viewrow').on('change', function(e){
		loadDataTable(0, 'serverTable1', "jsondata/getSalesListTest/Confirmed/");
	});


	appendServerTable("serverTable2", "jsondata/getSalesListTest/Created/", {'xdate':"Date", "xsonum":"Invoice Number", "xcus":"Customer ID", "xorg":"Org/Name"});
	$("#serverTable2-search").on("change paste input", function(){
		var search = $('#serverTable2-search').val();
		if(search.length > -1)
			loadDataTable(0, 'serverTable2', "jsondata/getSalesListTest/Created/");
	});
	$('#serverTable2-viewrow').on('change', function(e){
		loadDataTable(0, 'serverTable2', "jsondata/getSalesListTest/Created/");
	});
...
function appendServerTable(divid,givenurl, colarr= Array()){
	
	var defTable = '<div id="'+divid+'loadid" style="display:none;position:fixed;top:0;left:0;height:100%;width:100%;background: rgba( 255, 255, 255, .8 ) url(\'http://i.stack.imgur.com/FhHRx.gif\') 50% 50% no-repeat;opacity: 0.80;-ms-filter: progid:DXImageTransform.Microsoft.Alpha(Opacity = 80);filter: alpha(opacity = 80)"></div>'+
					'<div class="pull-left">'+
						'<select class="form-control" id="'+divid+'-viewrow">'+
							'<option>5</option>'+
							'<option>10</option>'+
							'<option>50</option>'+
							'<option>100</option>'+
					...
					'<div class="pull-right">'+
						'<div class="form-group">'+
							'<input type="text" class="form-control" id="'+divid+'-search">'+
						'</div>'+
					'</div>'+
					'<table class="table table-bordered" id="'+divid+'-invtable">'+
						'<thead>'+
							'<tr>';
				for (var key in colarr) {
				  	defTable+='<th>'+colarr[key]+'</th>';
				}
				...
				defTable+='<th>Show</th>'+
						...
						'</tbody>'+
					'</table>'+
					'<div class="pull-right" id="'+divid+'pageing">'+
					'</div>';
	
	$("#"+divid).append(defTable);
	loadDataTable(0,divid,givenurl,colarr);
}


function loadDataTable(index=0,divid,givenurl='',colarr=Array()){
	debugger;
	$('#'+divid+'loadid').show();
	var search = $('#'+divid+'-search').val();
	var url = baseuri+givenurl+index+"/"+$('#'+divid+'-viewrow').val()+"/"+search;
	$.get(url, function(q){
		debugger;
		addRowOnTable(divid+'-invtable',q,index,divid,givenurl,colarr);
	}, 'json').done(function() {
		$('#'+divid+'loadid').show();
	  }).fail(function() {
		alert( "error" );
...
function addRowOnTable(table='', q=Array(), index, divid, givenurl,colarr=Array()){
	debugger;
	var div='';
	for(var i=0;i<q['data'].length;i++){	
		div += '<tr role="row" class="odd">';
		for(var ke in colarr){
			div += '<td style="padding: 12px;">'+q['data'][i][ke]+'</td>';
		}
		
		div += '<td><input type="button" href="#tab_1" data-toggle="tab" value="Show" class="btn btn-info" onclick="clickfunction(\''+q['data'][i].xsonum+'\');" aria-expanded="true">'+
		'</td>'+
		'</tr>';
	}
	$('#'+table+' tbody').empty();
	...
	debugger;
	var prow = $('#'+divid+'-viewrow').val();
	var tpage = Math.ceil(q['totalrow'] / prow);
	var pageing = '';
	var p = 1;
	var li = 0;
	pageing = '<nav aria-label="Page navigation">'+
				'<ul class="pagination">';
				if((index-prow)<0){
					pageing += '<li class="disabled"><a href="avascript:void(0)" aria-label="Previous">'+
						'<span aria-hidden="true">&laquo;</span>'+
					'</a></li>';
				}else{
					pageing += '<li><a href="avascript:void(0)" onclick="loadDataTable(\''+(index-prow)+'\',\''+divid+'\',\''+givenurl+'\','+colarr+');" aria-label="Previous">'+
						'<span aria-hidden="true">&laquo;</span>'+
					'</a></li>';
				}
				do{
					if(tpage>5){
						if(p==1 || p==tpage || li==index || li==(parseInt(index)-parseInt(prow)) || li==(parseInt(index)+parseInt(prow))){
							pageing += '<li class="" id="'+divid+'li-'+li+'"><a href="avascript:void(0)" id="'+divid+'aa-'+li+'" onclick="loadDataTable(\''+li+'\',\''+divid+'\',\''+givenurl+'\','+colarr+');">'+p+'</a></li>';
						}else if(li==(parseInt(index)+parseInt(prow)+parseInt(prow)) || li==(parseInt(index)-parseInt(prow)-parseInt(prow))){
							pageing += '<li class="disabled" id="'+divid+'li-'+li+'"><a href="avascript:void(0)">...</a></li>';
						}
					}else{
						pageing += '<li class="" id="'+divid+'li-'+li+'"><a href="avascript:void(0)" id="'+divid+'aa-'+li+'" onclick="loadDataTable(\''+li+'\',\''+divid+'\',\''+givenurl+'\','+colarr+');">'+p+'</a></li>';
					}
				...
				if((parseInt(index)+parseInt(prow))==li){
					pageing += '<li class="disabled"><a href="avascript:void(0)" aria-label="Next">'+
					'<span aria-hidden="true">&raquo;</span>'+
					'</a></li>';
				}else{
					pageing += '<li><a href="avascript:void(0)" onclick="loadDataTable(\''+(parseInt(index)+parseInt(prow))+'\',\''+divid+'\',\''+givenurl+'\','+colarr+');" aria-label="Next">'+
					'<span aria-hidden="true">&raquo;</span>'+
					'</a></li>';
				}
				pageing += '</ul>'+
	...
	$('#'+divid+'pageing').empty();
	$('#'+divid+'pageing').append(pageing);
	$('#'+divid+'li-'+index).addClass('active');
}
...
function showInvoice(son){
	...
	var url = baseuri+"jsondata/getSoDtFree/"+sonum;
		...
		// var utl2 = baseuri+"jsondata/getSoItemStaFree/"+sonum;
		// $.get(utl2, function(q){ 
		// 	var tqty = 0;
		// 	var ttotal = 0;
		// 	if(q.length > 0)
...
function loadStaTable(){
	$('#StaTable').appendGrid({
        caption: 'Add Stationary for Issue',
        initRows: 10,
        columns: [
			...
			{ name: 'xcur', display: 'Curr.', ctrlClass: 'text-center', type: 'hidden', ctrlAttr: { maxlength: 100 }, ctrlCss: { width: '40px'} },
            { name: 'xmrp', display: 'Price', type: 'hidden', ctrlClass: 'xpricecls text-center', ctrlAttr: { maxlength: 100 }, ctrlCss: { width: '70px'},
			onChange: function (evt, rowIndex) {
				var sd = (event.target.id).replace(/[^0-9]/gi, '');
				var lid = parseInt(sd, 10);
				getNetTotalSta(lid);
			...
			{ name: 'xcurconversion', display: 'Convt.', ctrlClass: 'text-center', type: 'hidden', ctrlAttr: { maxlength: 100 }, ctrlCss: { width: '50px'},
			onChange: function (evt, rowIndex) {
				var sd = (event.target.id).replace(/[^0-9]/gi, '');
				var lid = parseInt(sd, 10);
				getNetTotalSta(lid);
			...
			{ name: 'xdis', display: 'Dis.', value:0, ctrlClass: 'text-center', type: 'hidden', ctrlAttr: { maxlength: 100 }, ctrlCss: { width: '40px'},
			onChange: function (evt, rowIndex) {
				var sd = (event.target.id).replace(/[^0-9]/gi, '');
				var lid = parseInt(sd, 10);
				getNetTotalSta(lid);
...
function printdoc(id){
	...
	$.get(utl, function(q){
		...
		if(bookcount > 0){
			...
			'<thead><tr role="row"><th style="width:60px;background-color: #95B72B!important;">Qty</th><th style="width:100px;background-color: #95B72B!important;">Item Code</th><th style="background-color: #95B72B!important;" >Item Name</th><th style="width:60px;background-color: #95B72B!important;">Price</th><th style="width:100px;background-color: #95B72B!important;">Total</th></tr></thead><tbody>';
			for(var i=0;i<q.length;i++){
				if(1==1){
					...
					div+='<td align="right">'+parseFloat(parseFloat(q[i].xqty)*parseFloat(q[i].xrate)).toFixed(bizdecimals)+'</td>';
					div+='</tr>';
			...
			div+='<tr><td align="center">'+bkqty+'</td><td colspan="3" align="right">Sub Total : </td><td align="right">'+parseFloat(bktotal).toFixed(bizdecimals)+'</td></tr></tbody></table>';
		}
		// var stacount = 0;
		// for(var i=0;i<q.length;i++){
		// 	if(q[i].xgitem != "Book"){
		...
		// 	div += '<table class="table table-bordered" cellspacing="0" width="100%" role="grid" aria-describedby="dtbl_info" style="width: 100%; font-size:11px;">'+
		// 	'<thead><tr role="row"><th style="width:100px">Item Code</th><th style="width:300px" >Description</th><th>Cur</th><th>Price</th><th>Convt.</th><th>N.Dis</th><th>N.Price</th><th>Qty</th><th>Total</th></tr></thead><tbody>';
		// 	for(var i=0;i<q.length;i++){
		// 		if(q[i].xgitem != "Book"){
		// 			debugger;
		// 			//var tval = (parseFloat(q[i].xmrp)*parseFloat(q[i].xqty)*parseFloat(q[i].xcurconversion));
		...
		// 		}
		// 	}
		// 	div+='<tr><td colspan="7" align="right">Sub Total : </td><td align="center">'+stqty+'</td><td align="center">'+sttotal+'</td></tr></tbody></table>';
		// }
		...
		// 		'<td align="right" style="border:1px solid"><p style="margin:2px">'+((total+xpackcharge+xtranscost-xgrossdisc)+(parseFloat(q[0].xcusbal))-(parseFloat(q[0].xrcvamt)))*(-1)+'</p></td>'+
		// 	'</tr>'+
		// '</table>'+
		// '<p style="margin: 10px 0px 0px 0px;"><b>In Word :</b> '+getText(q[0].xrcvamt)+'</p></div>';

		// div += '<div class="summary-table"><table style="width:40%; margin-top:0px; font-size:12px;line-height: 1.4;font-weight: bold;" class="pull-right">'+
		// 	'<tr style="border:1px solid">'+
		// 		'<td style="border:1px solid"><p style="margin:2px">Total Price : </p></td>'+
		// 		'<td align="right" style="border:1px solid"><p style="margin:2px">'+total+'</p></td>'+
		// 	'</tr>'+
		// 	'<tr style="border:1px solid">'+

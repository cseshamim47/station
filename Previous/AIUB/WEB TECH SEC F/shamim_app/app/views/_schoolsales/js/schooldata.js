...
function getCheckedSta(){
	debugger;
	var cus = $('#xcus').val();
	var clsarr = "";
	$('input.stafrmchecked:checkbox:checked').each(function () {
		clsarr += "'"+($(this).val())+"',";
	});
	clsarr = clsarr.slice(0, -1);
	var conv = $('#xconvertfor').val();
	var dis = $('#xdiscfor').val();
	getItemByCusSta(cus,clsarr,conv,dis);
}
...
function getNetTotalForNetDis(lid){
	...
	$('#ItemTable_xnet_'+lid).val(nprice);
	$('#ItemTable_xdis_'+lid).val((100*xnetdis)/stotal);
	$('#ItemTable_xtotal_'+lid).val(nprice*qty);
	//$('#ItemTable_xnetdis_'+lid).val(stotaldis);

	// var total = ((rate*convert)*qty);
	// var totaldis = (((rate*convert)*qty)*xnetdis);
	// var nettotal = total - xnetdis;
	//$('#ItemTable_xtotal_'+lid).val(nettotal);
	$('#ItemTable_xdistotal_'+lid).val(xnetdis*qty);
};
function getEnterNetPrice(lid){
	var rate = $('#ItemTable_xmrp_'+lid).val();
	var convert = $('#ItemTable_xcurconversion_'+lid).val();
	var qty =  $('#ItemTable_xqty_'+lid).val();
	var xdis =  $('#ItemTable_xdis_'+lid).val();
	var xnetdis =  $('#ItemTable_xnetdis_'+lid).val();
	var xnet =  $('#ItemTable_xnet_'+lid).val();

	var stotal = (rate*convert - xnet);
	$('#ItemTable_xnetdis_'+lid).val(stotal);
	$('#ItemTable_xtotal_'+lid).val(xnet*qty);
	$('#ItemTable_xdistotal_'+lid).val(stotal*qty);
	$('#ItemTable_xdis_'+lid).val((100*stotal)/(rate*convert));
...
function getEnterNetPriceSta(lid){
	...
	var xnetdis =  $('#StaTable_xnetdis_'+lid).val();
	var xnet =  $('#StaTable_xnet_'+lid).val();

	var stotal = (rate*convert - xnet);
	$('#StaTable_xnetdis_'+lid).val(stotal);
	$('#StaTable_xtotal_'+lid).val(xnet*qty);
	$('#StaTable_xdistotal_'+lid).val(stotal*qty);
	$('#StaTable_xdis_'+lid).val((100*stotal)/(rate*convert));
...
function getNetTotalForNetDisSta(lid){
	var rate = $('#StaTable_xmrp_'+lid).val();
	var convert = $('#StaTable_xcurconversion_'+lid).val();
	var qty =  $('#StaTable_xqty_'+lid).val();
	var xdis =  $('#StaTable_xdis_'+lid).val();
	...
	var nprice = stotal - xnetdis;
	$('#StaTable_xnet_'+lid).val(nprice);
	$('#StaTable_xdis_'+lid).val((100*xnetdis)/stotal);
	$('#StaTable_xtotal_'+lid).val(nprice*qty);
	//$('#StaTable_xnetdis_'+lid).val(stotaldis);
	...
	// var nettotal = total - xnetdis;
	//$('#StaTable_xtotal_'+lid).val(nettotal);
	$('#StaTable_xdistotal_'+lid).val(xnetdis*qty);
};
...
function getItemByCusSta(cus,clsarr,conv,dis){
	debugger;
	var utl = baseuri+"jsondata/getAllItemSta/"+cus+"/"+clsarr+"/"+conv+"/"+dis;
	$.get(utl, function(q){ 
		debugger;
		$('#StaTable').appendGrid('load', q);
		getTotalSalesAmount();
...
function getClass(cus){
	var url = baseuri+"jsondata/getclass/"+cus+"/Book";
	$.get(url, function(o){ 
				debugger;
				$( "#appenddiv" ).empty();
				$( "#appenddiv" ).append( '<div id="row1" class="form-group"><label for="Class" class="col-sm-2 '+'control-label"></label><div class="col-sm-12"><div style="Background-color:#00a6cc; '+'color:white"><strong>Select Class For Book</strong></div></div></div>'+
				'' );
...
function getClassSta(cus){
	var url = baseuri+"jsondata/getclass/"+cus+"/Stationary";
	$.get(url, function(o){ 
				debugger;
				$( "#appenddivSta" ).empty();
				$( "#appenddivSta" ).append( '<div id="row2" class="form-group"><label for="Class" class="col-sm-2 '+'control-label"></label><div class="col-sm-12"><div style="Background-color:#00a6cc; '+'color:white"><strong>Select Class For Stationary</strong></div></div></div>'+
				'' );
				for(var i=0; i< o.length; i ++){
					$( "#appenddivSta" ).append( '<label for="sta'+ o[i].xclass +'" class="col-sm-2 control-label">'+ o[i].xclass +'</label>'+
					'<div class="col-sm-1">'+
						'<div class="staclscheck" id="staclscheck"><label><input onclick="getCheckedSta()" name="check_liststa[]" class="stafrmchecked" id="sta'+ o[i].xclass +'" value="'+ o[i].xclass +'" type="checkbox"></label>'+
						'</div>'+
					'</div>' );
				}
		}, 'json');
...
function loadBookTable(){
	$('#ItemTable').appendGrid({
        ...
        columns: [
            ...
            { name: 'xmrp', display: 'Price', type: 'number', ctrlClass: 'xpricecls text-center', ctrlAttr: { maxlength: 100 }, ctrlCss: { width: '70px'}, ctrlProp: { readonly: 'readonly' },
			onChange: function (evt, rowIndex) {
				var sd = (event.target.id).replace(/[^0-9]/gi, '');
				var lid = parseInt(sd, 10);
				getNetTotal(lid);
			...
			{ name: 'xdis', display: 'Dis.', value:0, type: 'number', ctrlClass: 'text-center', ctrlAttr: { maxlength: 100 }, ctrlCss: { width: '40px'},
			onChange: function (evt, rowIndex) {
				var sd = (event.target.id).replace(/[^0-9]/gi, '');
				var lid = parseInt(sd, 10);
				getNetTotal(lid);
			...
			{ name: 'xnetdis', display: 'N.Dis.', value:0, type: 'number', ctrlClass: 'text-center', ctrlAttr: { maxlength: 100 }, ctrlCss: { width: '50px'},
			onChange: function (evt, rowIndex) {
				var sd = (event.target.id).replace(/[^0-9]/gi, '');
				var lid = parseInt(sd, 10);
			...
			{ name: 'xnet', display: 'N.Price', ctrlClass: 'text-center', type: 'number', ctrlAttr: { maxlength: 100 }, ctrlCss: { width: '60px'},
			onChange: function (evt, rowIndex) {
				var sd = (event.target.id).replace(/[^0-9]/gi, '');
				var lid = parseInt(sd, 10);
				getEnterNetPrice(lid);
				getTotalSalesAmount();
			} },
...
function printdoc(id){
	...
	$.get(utl, function(q){
		...
		div += '<div class="main-div" style="margin-top:100px;"><div style="text-align:center"><strong align="center" style="border-bottom:1px solid;">Invoice</strong></div><table class="table table-bordered" cellspacing="0" width="100%" role="grid" aria-describedby="dtbl_info" style="width: 100%; font-size:12px; margin-top:10px;">';
		div += '<tr><td style="background-color:#d8d8d8!important;width: 100px;">Invoice No.</td><td>'+sonum+'</td><td style="background-color:#d8d8d8!important;width: 80px;">Customer ID</td><td>'+q[0].xcus+'</td><td style="background-color:#d8d8d8!important;width: 85px;">Invoice Date</td><td>'+sdt+'</td></tr>';
		div += '<tr><td style="background-color:#d8d8d8!important;">Customer Name</td><td colspan="3">'+q[0].xorg+'</td><td style="background-color:#d8d8d8!important;">Phone</td><td colspan="">'+q[0].xphone+'</td></tr>';
		div += '<tr><td style="background-color:#d8d8d8!important;">Divition</td><td>'+q[0].xcity+'</td><td style="background-color:#d8d8d8!important;">District</td><td>'+q[0].xprovince+'</td><td style="background-color:#d8d8d8!important;">Mobile</td><td colspan="">'+q[0].xmobile+'</td></tr>';
		div += '<tr><td style="background-color:#d8d8d8!important;">Address</td><td colspan="5">'+q[0].xadd1+'</td></tr></table>';
		...
		if(bookcount > 0){
			...
			'<thead><tr role="row"><th style="width:60px;background-color: #95B72B!important;">Qty</th><th style="width:80px;background-color: #95B72B!important;">Item Code</th><th style="width:325px;background-color: #95B72B!important;" >Description</th><th style="width:30px;background-color: #95B72B!important;">Cur</th><th style="width:60px;background-color: #95B72B!important;">Price</th><th style="width:30px;background-color: #95B72B!important;">Convt.</th><th style="width:60px;background-color: #95B72B!important;">N.Dis</th><th style="width:65px;background-color: #95B72B!important;">Sale Price</th><th style="width:100px;background-color: #95B72B!important;">Total</th></tr></thead><tbody>';
			for(var i=0;i<q.length;i++){
				if(q[i].xgitem == "Book"){
					...
					div+='<td align="right">'+parseFloat(q[i].xtotal).toFixed(bizdecimals)+'</td>';
					div+='</tr>';
			...
			div+='<tr><td align="center">'+bkqty+'</td><td colspan="7" align="right">Sub Total : </td><td align="right">'+parseFloat(bktotal).toFixed(bizdecimals)+'</td></tr></tbody></table>';
		}
		var stacount = 0;
		for(var i=0;i<q.length;i++){
			if(q[i].xgitem != "Book"){
		...
		if(stacount > 0){
			div += '<div style="margin-top:0px;" height:500px;><strong style="">Stationary : </strong></div>';
			div += '<table class="table table-bordered" cellspacing="0" width="100%" role="grid" aria-describedby="dtbl_info" style="width: 100%; font-size:11px;">'+
			'<thead><tr role="row"><th style="width:60px;background-color: #95B72B!important;">Qty</th><th style="width:80px;background-color: #95B72B!important;">Item Code</th><th style="width:325px;background-color: #95B72B!important;" >Description</th><th style="width:30px;background-color: #95B72B!important;">Cur</th><th style="width:60px;background-color: #95B72B!important;">Price</th><th style="width:30px;background-color: #95B72B!important;">Convt.</th><th style="width:60px;background-color: #95B72B!important;">N.Dis</th><th style="width:65px;background-color: #95B72B!important;">Sale Price</th><th style="width:100px;background-color: #95B72B!important;">Total</th></tr></thead><tbody>';
			for(var i=0;i<q.length;i++){
				if(q[i].xgitem != "Book"){
					...
					div+='<td align="center">'+(parseFloat(q[i].xdistotal)/parseFloat(q[i].xqty))+'</td>';
					div+='<td align="center">'+((parseFloat(q[i].xmrp) * parseFloat(q[i].xcurconversion)) - (parseFloat(q[i].xdistotal) / parseFloat(q[i].xqty)))+'</td>';
					div+='<td align="right" width="10px">'+parseFloat(q[i].xtotal).toFixed(bizdecimals)+'</td>';
					div+='</tr>';
			...
			div+='<tr><td align="center">'+stqty+'</td><td colspan="7" align="right">Sub Total : </td><td align="right">'+parseFloat(sttotal).toFixed(bizdecimals)+'</td></tr></tbody></table>';
		}
		...
		div += '<div class="summary-table pull-left" style="width:55%;"><table style="width:60%; margin-top:0px; font-size:12px;line-height: 1.4;font-weight: bold;" class="">'+
			'<tr style="border:1px solid">'+
				...
				'<td align="right" style="border:1px solid"><p style="margin:2px">'+parseFloat(parseFloat(q[0].xcusbal)*-1).toFixed(bizdecimals)+'</p></td>'+
			'</tr>'+
			// '<tr>'+
			// 	'<td>Invoice Total : </td>'+
			...
			'<tr style="border:1px solid">'+
				...
				'<td align="right" style="border:1px solid"><p style="margin:2px">'+parseFloat(q[0].xrcvamt).toFixed(bizdecimals)+'</p></td>'+
			'</tr>'+
			'<tr style="border:1px solid">'+
				'<td style="border:1px solid"><p style="margin:2px">Total Due : </p></td>'+
				'<td align="right" style="border:1px solid"><p style="margin:2px">'+parseFloat(((total+xpackcharge+xtranscost-xgrossdisc)+(parseFloat(q[0].xcusbal))-(parseFloat(q[0].xrcvamt)))*(-1)).toFixed(bizdecimals)+'</p></td>'+
			'</tr>'+
		'</table>'+
		'<p style="margin: 10px 0px 0px 0px;"><b>In Word :</b> '+getText(q[0].xrcvamt)+'</p></div>';

		div += '<div class="summary-table"><table style="width:35%; margin-top:-20px; font-size:12px;line-height: 1.4;font-weight: bold;" class="pull-right">'+
			'<tr style="">'+
				'<td style="border-left:1px solid;border-right:1px solid"><p style="margin:2px">Total Price : </p></td>'+
				'<td align="right" style="border-right:1px solid; width:95px;"><p style="margin:2px">'+parseFloat(total).toFixed(bizdecimals)+'</p></td>'+
			'</tr>'+
			'<tr style="border:1px solid">'+
				'<td style="border:1px solid"><p style="margin:2px">Packing Charge : </p></td>'+
				'<td align="right" style="border:1px solid"><p style="margin:2px">'+parseFloat(xpackcharge).toFixed(bizdecimals)+'</p></td>'+
			'</tr>'+
			'<tr style="border:1px solid">'+
				'<td style="border:1px solid"><label for="xtranscost" class="control-label"><p style="margin:2px">Transport Cost : </p></label></td>'+
				'<td align="right" style="border:1px solid"><p style="margin:2px">'+parseFloat(xtranscost).toFixed(bizdecimals)+'</p></td>'+
			'</tr>'+
			'<tr style="border:1px solid">'+
				'<td style="border:1px solid"><p style="margin:2px">Sub Total Amount : </p></td>'+
				'<td align="right" style="border:1px solid"><p style="margin:2px">'+parseFloat(total+xpackcharge+xtranscost).toFixed(bizdecimals)+'</p></td>'+
			'</tr>'+
			'<tr style="border:1px solid">'+
				'<td style="border:1px solid"><p style="margin:2px">Less : </p></td>'+
				'<td align="right" style="border:1px solid"><p style="margin:2px">'+parseFloat(xgrossdisc).toFixed(bizdecimals)+'</p></td>'+
			'</tr>'+
			'<tr style="border:1px solid">'+
				'<td style="border:1px solid"><p style="margin:2px">Net Bill : </p></td>'+
				'<td align="right" style="border:1px solid;"><p style="margin:2px">'+parseFloat(total+xpackcharge+xtranscost-xgrossdisc).toFixed(bizdecimals)+'</p></td>'+
			'</tr>'+
		'</table></div>';

$(function(){
	...
	//$( "#datetest" ).datepicker();
	$( "#datetest" ).datepicker( "option", "dateFormat", 'dd/mm/yy' );
...
function loadSOTable(status){
	var url = baseuri+"jsondata/getSalesListReject/"+status;
		$.get(url, function(q){
			debugger;
...
function showInvoice(son){
	debugger;
	var sonum = son;
	var url = baseuri+"jsondata/getSoDtReject/"+sonum;
		$.get(url, function(q){
			debugger; 
			if(q.length < 1){
...
function deleteRow(tblname,lid){
	...
	if(rownum != ""){
		var utl2 = baseuri+"reject/deleteRow/"+sonum+"/"+rownum;
		$.get(utl2, function(q){
			debugger;
			alert(q);
			// if(q == 'Success')
...
function btnconfclick(){
	...
	if(confirm("Are you sure you want to Confirm this Invoice?")){
		...
		var utl2 = baseuri+"reject/confirmpost/"+sonum+"/"+disc;
		$.get(utl2, function(q){ 
			if(q="Success"){
				$("#confirmbtn").empty();
				$('<a href="javascript:void(0)" id="btncancel" onclick="btncancelclick();" class="btn btn-danger"><span class="glyphicon glyphicon-save"></span>&nbsp;Cencel Sales</a>').appendTo("#confirmbtn");
...
function btncancelclick(){
	...
	if(confirm("Are you sure you want to Cancel this Invoice?")){
		...
		// var org = $('#xorg').val();
		var disc = $('#xgrossdisc').val();
		var utl2 = baseuri+"reject/cancelpost/"+sonum+"/"+disc;
		$.get(utl2, function(q){ 
			if(q="Success"){
				$("#confirmbtn").empty();
...
function loadBookTable(){
	...
	$('#ItemTable').appendGrid({
        ...
        initRows: 10,
        columns: [
			{ name: 'xitemcode', ctrlClass: '', display: 'Item Code', type: 'text', ctrlAttr: { maxlength: 100 }, ctrlCss: { width: '200px'}},
			{ name: 'StockIn', display: 'Stock In', value: strDate, type: 'ui-datepicker', ctrlAttr: { maxlength: 10 }, ctrlCss: { width: '100px' }, uiOption: { dateFormat: 'yy/mm/dd'} },
            { name: 'xdesc', ctrlClass: 'example-mail', display: 'Description(English)', type: 'text', ctrlAttr: { maxlength: 250 }, ctrlCss: { width: '600px'}, ctrlProp: {  },
...
//             { name: 'xlongdesc', display: 'Description(Bangla)', type: 'text', ctrlAttr: { maxlength: 250 }, ctrlCss: { width: '370px'}, ctrlProp: {  },
...
// 			}  },
            ...
            { name: 'xqty', display: 'Qty', ctrlClass: 'text-center bkqty', type: 'number', ctrlAttr: { maxlength: 100 }, ctrlCss: { width: '95px'},
			onChange: function (evt, rowIndex) {
				var sd = (event.target.id).replace(/[^0-9]/gi, '');
				var lid = parseInt(sd, 10);
				getItemdt(lid);
...
function printdoc(id){
	...
	$.get(utl, function(q){
		...
		div += '<div class="main-div" style="margin-top:100px;"><div style="text-align:center"><strong align="center" style="border-bottom:1px solid;">Free Issue Invoice</strong></div><table class="table table-bordered" cellspacing="0" width="100%" role="grid" aria-describedby="dtbl_info" style="width: 100%; font-size:11px; margin-top:10px;"><tbody>';
		...
		if(bookcount > 0){
			...
			div+='<tr><td colspan="2" align="right">Sub Total : </td><td align="center">'+bkqty+'</td></tr></tbody></table>';
		}
		// var stacount = 0;
		// for(var i=0;i<q.length;i++){
		// 	if(q[i].xgitem != "Book"){

...
function loadSOTable(status){
	var url = baseuri+"submittradepay/getBillList/"+status;
		$.get(url, function(q){
			debugger;
	var div = '<table id="dtbl'+status+'" class="table table-bordered table-striped">'+
		...
		for(var i=0;i<q.length;i++){	
			...
			'<td><input type="button" href="#tab_1" data-toggle="tab" id="'+q[i].xtxnnum+'" value="Show" class="btn btn-info" onclick="clickfunction(\''+q[i].xtxnnum+'\');" aria-expanded="true">'+
			'</td>'+
			'</tr>';
		}
...
function showInvoice(son){
	debugger;
	var sonum = son;
	var url = baseuri+"submittradepay/getBillMst/"+sonum;
		$.get(url, function(q){
			debugger; 
			if(q.length < 1){
		...
		var utl = baseuri+"submittradepay/getBillDet/"+sonum;
		$.get(utl, function(q){ 
			var tqty = 0;
			var ttotal = 0;
			debugger;
...
function deleteRow(tblname,lid){
	...
	if(rownum != ""){
		var utl2 = baseuri+"submittradepay/deleteRow/"+sonum+"/"+rownum;
		$.get(utl2, function(q){
			debugger;
			alert(q);	
		}, 'json');
...
function btnconfclick(){
	...
	if(confirm("Are you sure you want to Confirm this Bill?")){
		$('#processing').show();
        var sonum = $('#xtxnnum').val();
		var utl2 = baseuri+"submittradepay/confirmpost/"+sonum;
		$.get(utl2, function(q){ 
			$('#processing').hide();
			if(q="Success"){
				$("#confirmbtn").empty();
...
function btncancelclick(){
	...
	if(confirm("Are you sure you want to Cancel this Bill?")){
		var sonum = $('#xtxnnum').val();
		var utl2 = baseuri+"submittradepay/cancelpost/"+sonum;
		$.get(utl2, function(q){ 
			if(q="Success"){
				$("#confirmbtn").empty();
...
function loadBookTable(subval){
	...
	$('#ItemTable').appendGrid({
        ...
        afterRowAppended: function (caller, parentRowIndex, addedRowIndex) {
			...
			var options = {
				url: "submittradepay/getItemData",
...
function printdoc(id){
	...
	var utl = baseuri+"submittradepay/getBillForPrint/"+sonum;
	$.get(utl, function(q){
		debugger;

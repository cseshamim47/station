<?php
class Schoolsales extends Controller{
	...
	function __construct(){
		...
		$this->view->js = array('views/schoolsales/js/jquery-1.12.4.min.js','views/schoolsales/js/jquery-ui-1.12.1.min.js','views/schoolsales/js/jquery.appendGrid-1.7.1.js','views/schoolsales/js/schooldata.js','views/schoolsales/js/jQuery.print.js','views/schoolsales/js/formpost.js','views/schoolsales/js/EasyAutocomplete/jquery.easy-autocomplete.min.js','public/js/default.js','public/theme/bower_components/bootstrap-datepicker/dist/js/bootstrap-datepicker.min.js','public/js/jquery.dataTables.min.js','public/js/dataTables.bootstrap.min.js');
		$dt = date("Y/m/d");
		...
		public function savesales(){
			...
			if($status == "New" || $status == "Created"){
				...
				// fclose($file);
				$rownum = $this->model->getRow("sodet","xsonum",$keyfieldval,"xrow");
				...
				$cols = "`bizid`,`xsonum`,`xrow`,`xitemcode`,`xitembatch`,`xqty`,`xdate`,`xcost`,`xrate`,`xwh`,`xbranch`,`xproj`,`xtaxrate`,`xdisc`,`xtaxcode`,`xtaxscope`,`zemail`,`xunitsale`,`xcus`,`xyear`,`xper`,`xtypestk`,`xdispct`,`xexch`";
		...
		function confirmpost($sonum,$disc,$cus){
			...
			if(!empty($glinterface)){
				...
				$month = date('m',strtotime($date)); 
					...
					$data['xnarration'] = "C/S SO:".$sonum." Cus: ".$cus."-".$org;
					$data['xyear'] = $xyear;
					$data['xper'] = $per;
	...
	function cancelpost($sonum,$disc,$cus){
		...
		if(!empty($glinterface)){
			...
			$month = date('m',strtotime($date)); 
				...
				$data['bizid'] = Session::get('sbizid');
				$data['zemail'] = Session::get('suser');
				$data['xnarration'] = "C/S CSO:".$sonum." by ". Session::get('suser') ." Cus: ".$cus."-".$org;
				$data['xyear'] = $xyear;
				$data['xper'] = $per;
				$data['xvoucher'] = $voucher;
				$data['xdate'] = $date;

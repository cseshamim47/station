<?php
class Alltada extends Controller{
	...
	public function index($flag=""){
		...
		$this->view->rendermainmenu('alltada/index');	
	}
	...
	function datewisepo(){
		...
		$values = array(
			...
			$this->view->dynform = $dynamicForm->createFourColumnFormGen($fields, URL."alltada/datewisepo", "Show",$values);
			
			$this->view->table = $this->renderTable($fdate, $tdate, $xcat);
			
			$this->view->renderpage('alltada/porptfilter', false);
	}
	...
	function renderTable($fdate, $tdate, $xcat){
		...
		//print_r($row); die;
		return $this->createTable($fields, $row, "xtxnnum", URL."alltada/showreq/", URL."alltada/approve/", URL."alltada/cancel/");
		
		
	}

	function showreq($reqnum){
		$this->view->backUrl=URL."alltada/datewisepo";

		$reqmst = $this->model->getSingleReq($reqnum);
		$this->view->reqmst = $reqmst;
		$this->view->table=$this->renderTable2($reqnum);
		$this->view->renderpage('alltada/itemreceivelist', false);
	}
	...
	function renderTable2($reqnum){
		...
		function picklist(){
			...
			$this->view->renderpage('alltada/picklist', false);
		}

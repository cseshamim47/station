				...
				if(!empty($accdetail))
					if($_POST['type']=="Supplier Payment"){
						...
						if(!empty($supayment)){
							...
							$data['xnarration'] = $supayment[0]['xnote'];
							$data['xyear'] = $year;
							$data['xper'] = $month;
						...
						if(!empty($wopayment)){
							...
							$data['bizid'] = Session::get('sbizid');
							$data['xvoucher'] = $keyfieldval;
							$data['xnarration'] = $wopayment[0]['xnote'];
							$data['xyear'] = $year;
							$data['xper'] = $month;
							$data['xbranch'] = Session::get('sbranch');
							$data['xdoctype'] = "Payment Voucher";
							...
							$vals .= "(" . Session::get('sbizid') . ",'". $keyfieldval ."','1','9000','Liability','AP','Sub Account','".$wo[0]['wonum']."','BDT','".$wopayment[0]['xamount']."','1','".$wopayment[0]['xamount']."','Debit'),";

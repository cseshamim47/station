<?php
class Billrpt extends Controller{
	...
	function __construct(){
		...
		foreach($usersessionmenu as $menus){
				if($menus["xsubmenu"]=="Bill Reports")
					$iscode=1;				
		}
		if($iscode==0)
			header('location: '.URL.'mainmenu');
			
		$this->view->js = array('public/js/datatable.js','views/billrpt/js/getaccdt.js');
								
		}
		
		public function index(){
			$this->view->rendermainmenu('billrpt/index');
		}
		...
		function porptmenu(){
			...
			$menuarr = array(
			0 => array("Date wise Bill"=>URL."billrpt/datewisebill","Category wise Bill"=>URL."billrpt/catwisebill","Project wise Bill"=>URL."billrpt/prowisebill"),
			...
			$this->view->renderpage('billrpt/porptmenu', false);
		}
		...
		function datewisebill(){
			...
			$this->view->breadcrumb = '<ul class="breadcrumb">
							<li><a href="'.URL.'billrpt/porptmenu">Bill Reports</a></li>
							<li class="active">Report Filter</li>
						   </ul>';
			$btn = array(
				"Clear" => URL."billrpt/datewisebill",
			);	
			$dt = date("Y/m/d");
			...
			$values = array(
				...
				$this->view->dynform = $dynamicForm->createFourColumnFormGen($fields, URL."billrpt/showdatewisebill", "Show Report",$values);
				
				$this->view->table = "";
				
				$this->view->renderpage('billrpt/porptfilter', false);
		...
		public function showdatewisebill(){
			...
			$this->view->breadcrumb = '<ul class="breadcrumb">
							<li><a href="'.URL.'billrpt/porptmenu">Bill Reports</a></li>
							<li><a href="'.URL.'billrpt/datewisebill">Report Filter Form</a></li>
			...
			$tblsettings = array(
				...
				"grouprecords"=>array("Date~mstdate","Name~xsup","Project~xproject"), //database records columns to show in group
			...
			$this->view->renderpage('billrpt/reportpage');
		}
		...
		function prowisebill(){
			...
			$values = array(
				...
				$this->view->dynform = $dynamicForm->createFourColumnFormGen($fields, URL."billrpt/showprowisebill", "Show Report",$values);
		...
		public function showprowisebill(){
			...
			$this->view->breadcrumb = '<ul class="breadcrumb">
							<li><a href="'.URL.'billrpt/porptmenu">Bill Reports</a></li>
							<li><a href="'.URL.'billrpt/prowisebill">Report Filter Form</a></li>
							<li class="active">Bill List</li>
						   </ul>';
			...
			$this->view->org=Session::get('sbizlong');
			$tblsettings = array(
				"columns"=>array(0=>"Txn No","1"=>"Name","2"=>"Category","3"=>"Date","4"=>"Description",5=>"Quality",6=>"Price",7=>"Quentity",8=>"Amount",9=>"Remarks"),
				"groupfield"=>"Project~xproject",
				"grouprecords"=>array(), //database records columns to show in group
				"detailsection"=>array("xtxnnum","xsup","xcat","xdate","xdesc","xquality~center","xprice~center","xqty~center","xtotal~center","xremarks~center"),
				//"buttons"=>array("Show"=>URL."test/","Delete"=>URL."test/"),
				//"urlvals"=>array("id","phone"),
				"sumfields"=>array("xqty","xtotal"),
				);
		...
		function catwisebill(){
			...
			$values = array(
				...
				$this->view->dynform = $dynamicForm->createFourColumnFormGen($fields, URL."billrpt/showcatwisebill", "Show Report",$values);
		...
		public function showcatwisebill(){
			...
			$this->view->breadcrumb = '<ul class="breadcrumb">
							<li><a href="'.URL.'billrpt/porptmenu">Bill Reports</a></li>
							<li><a href="'.URL.'billrpt/catwisebill">Report Filter Form</a></li>
							<li class="active">Bill List</li>
						   </ul>';
			...
			$cond = "";
			if($xcat != ""){
			    $cond .= "</br>Category : ".$xcat;
			}
			
			if($xproject != ""){
			    $cond .= "</br>Project : ".$xproject;
			}
				
			$tblvalues=array();			
			
				$tblvalues = $this->model->catWiseBill($fdate, $tdate, $xcat, $xproject);
				//print_r($tblvalues);die;
			
			$this->view->vfdate = $fdt;
			$this->view->vtdate = $tdt;
			$this->view->vrptname = "Category wise Bill".$cond;
			$this->view->org=Session::get('sbizlong');
			$tblsettings = array(
		...
		function supwisepo(){
			...
			$values = array(
				...
				$this->view->dynform = $dynamicForm->createFourColumnFormGen($fields, URL."billrpt/showsupwisepo", "Show Report",$values);
		...
		public function showsupwisepo(){
			...
			$this->view->breadcrumb = '<ul class="breadcrumb">
							<li><a href="'.URL.'billrpt/porptmenu">Bill Reports</a></li>
							<li><a href="'.URL.'billrpt/supwisepo">Report Filter Form</a></li>
							<li class="active">Bill List</li>
						   </ul>';

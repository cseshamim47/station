<?php
class Singlepresentreport extends Controller{
	...
	function __construct(){
		...
		foreach($usersessionmenu as $menus){
				if($menus["xsubmenu"]=="Employee Present Report")
					$iscode=1;				
		}
		if($iscode==0)
		...
		public function index(){
			$btn = array(
				"Clear" => URL."singlepresentreport/stuattentry",
			);
			$this->view->rendermainmenu('singlepresentreport/index');
		}
		...
		function studentlist(){
			...
			{
				header ('location: '.URL.'singlepresentreport/stuattentry');
				exit;
			}
			...
			$this->view->dynform = $dynamicForm->createForm($this->fields,"","",$frmvalues, URL."singlepresentreport/studentlist");
			...
			$this->view->renderpage('singlepresentreport/stuattentry', false);
		}
		...
		$btn = array(
			...
			$this->view->dynform = $dynamicForm->createForm($this->fields,"","",$this->values, URL."singlepresentreport/studentlist");
		...
		function renderTable($data){
			...
			$table.='<header style="font-size: 20px; text-align:center; margin-bottom:5px; font-weight:bold;">Employee Present Report</header>';
			$table.='<div>';
			...
			$table.='<tr><th>Date</th><th>In Time</th><th>Out Time</th></tr>';
			$table.='</thead>';
			$table.='<tbody>';
			$totalleave = 0;
			foreach ($data as $key => $value) {

<?php 
class Distreqgen extends Controller{
	...
	function __construct(){
		...
		foreach($usersessionmenu as $menus){
				if($menus["xsubmenu"]=="General Store Purchase Req.")
					$iscode=1;				
		}
		if($iscode==0)
			header('location: '.URL.'mainmenu');
			
		$this->view->js = array('public/js/datatable.js','views/distreqgen/js/codevalidator.js','views/distreqgen/js/getitemdt.js');
		$dt = date("Y/m/d");
		$this->values = array(
			"xdate"=>$dt,
			"ximreqnum"=>"",
		...
		$btn = array(
			"Clear" => URL."distreqgen/distreqentry",
		);	
		
					
			$this->view->rendermainmenu('distreqgen/index');
		...
		public function editdistreq(){
			...
			if (empty($_POST['ximreqnum'])){
					header ('location: '.URL.'distreqgen/distreqentry');
					exit;
				}
			$result = "";
		...
		$tblvalues=array();
		$btn = array(
			"New" => URL."distreqgen/distreqentry"
		);
		...
		if(count($tblvalues)>0){	
			if($tblvalues[0]['xstatus']=="Created" || $tblvalues[0]['xstatus']=="Canceled"){
				$conf=array(URL."distreqgen/confirmpost", "Confirm");
				$sv=URL."distreqgen/editdistreq";
			}		
			if($tblvalues[0]['xstatus']=="Pending" || $tblvalues[0]['xstatus']=="Confirmed"){
				$sv="";
				$conf=array(URL."distreqgen/cancelpost", "Cancel");
			}
		...
		// form data
			...
			$this->view->dynform = $dynamicForm->createForm($this->fields, $sv, "Update",$tblvalues, URL."distreqgen/showpost", $conf);
			
			$this->view->table = $this->renderTable($reqnum);
			$this->view->prevmodify = $prevmodify;
			$this->view->renderpage('distreqgen/distreqentry');
		}
		...
		if(empty($tblvalues))
			...
			$this->view->dynform = $dynamicForm->createForm($this->fields, $sv, "Save",$tblvalues ,URL."distreqgen/showpost", $conf);
		...
		function imreqlist($status){
			...
			$table = new Datatable();
			$this->view->table = $table->createTable($cols, $rows, "ximreqnum",URL."distreqgen/showentry/");
			$this->view->renderpage('distreqgen/distreqlist', false);
		}		
		...
		// form data
			...
			$this->view->dynform = $dynamicForm->createForm($this->fields, URL."distreqgen/savedistreq", "Save",$this->values, URL."distreqgen/showpost");
			
			
			$this->view->table = $this->renderTable("");
			
			$this->view->renderpage('distreqgen/distreqentry', false);
		}
		...
		if(count($tblvalues)>0){	
			if($tblvalues[0]['xstatus']=="Created"){
				$conf=array(URL."distreqgen/confirmpost", "Confirm");
				$sv=URL."distreqgen/savedistreq";
				$result='Created!<br/><a style="color:red" id="invnum" href="'.URL.'distreqgen/showquot/'.$imreqnum.'">Click To Print Requisition - '.$imreqnum.'</a>';
			}		
			if($tblvalues[0]['xstatus']=="Pending"){
				$sv="";
				$conf=array(URL."distreqgen/cancelpost", "Cancel");
				$result='Pending!<br/><a style="color:red" id="invnum" href="'.URL.'distreqgen/showquot/'.$imreqnum.'">Click To Print Requisition - '.$imreqnum.'</a>';
			}		
			if($tblvalues[0]['xstatus']=="Confirmed"){
				$sv="";
				$conf=array(URL."distreqgen/cancelpost", "Cancel");
				$result='Confirmed!<br/><a style="color:red" id="invnum" href="'.URL.'distreqgen/showquot/'.$imreqnum.'">Click To Print Requisition - '.$imreqnum.'</a>';
			}
			$prevmodify = $tblvalues[0]['xcomment'];
		}
		...
		function showquot($reqnum=""){
			...
			//print_r($this->model->getImreq($reqnum));
			...
			$this->view->renderpage('distreqgen/quotation');
		}
		...
		function confirmpost(){
			...
			if($tblvalues[0]['xstatus']!="Created"){
				$result='Confirmed!<br/><a style="color:red" id="invnum" href="'.URL.'distreqgen/showquot/'.$reqnum.'">Click To Print PO Requisition - '.$reqnum.'</a>';
			}
		...
		function cancelpost(){
			...
			if($tblvalues[0]['xstatus']=="Deleted"){
				$result='PO Requisition Canceled!<br/><a style="color:red" id="invnum" href="'.URL.'distreqgen/showquot/'.$reqnum.'">Click To Print PO Requisition - '.$reqnum.'</a>';
			}

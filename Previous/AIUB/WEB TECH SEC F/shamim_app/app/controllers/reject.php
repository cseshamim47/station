...
class Reject extends Controller{
	...
	function __construct(){
		...
		foreach($usersessionmenu as $menus){
				if($menus["xsubmenu"]=="Reject Entry")
					$iscode=1;				
		}
		if($iscode==0)
			header('location: '.URL.'mainmenu');
			
		$this->view->js = array('public/js/default.js','views/reject/js/jquery-1.12.4.min.js','views/reject/js/jquery-ui-1.12.1.min.js','views/reject/js/jquery.appendGrid-1.7.1.js','views/reject/js/schooldata.js','views/reject/js/jQuery.print.js','views/reject/js/formpost.js','views/reject/js/EasyAutocomplete/jquery.easy-autocomplete.min.js','public/js/jquery.dataTables.min.js','public/js/dataTables.bootstrap.min.js');
		$dt = date("Y/m/d");
		...
		public function savesales(){
			...
			if($_POST['xsonum'] == "")
				$keyfieldval = $this->model->getKeyValue("rejectmst","xsonum","IFRD-","5");
			else
			...
			if($status == "New" || $status == "Created"){
				...
				$rownum = $this->model->getRow("rejectdet","xsonum",$keyfieldval,"xrow");
				//Data made for Master Table rejectmst
				$mcols = "";
				$mvals = "";
				...
				$mvals = "(" . Session::get('sbizid') . ",'". $_POST['xwh'] ."','". $date ."','','0','0','','0','". $_POST['xnotes'] ."','". Session::get('suser') ."','". $keyfieldval ."',".$year.",".$month.",'Created','0','0')";
				...
				if($_POST['ItemTable_rowOrder']!=null && $_POST['ItemTable_rowOrder']!=""){
					...
					foreach($lbl as $k=>$v){
						...
						if($_POST['ItemTable_xitemcode_'.$v] != "" && $_POST['ItemTable_xqty_'.$v] != "" && $_POST['ItemTable_xrow_'.$v] != ""){
							...
							$vals .= "(" . Session::get('sbizid') . ",'". $keyfieldval ."','". $_POST['ItemTable_xrow_'.$v] ."','". $_POST['ItemTable_xitemcode_'.$v] ."','". $_POST['ItemTable_xitemcode_'.$v] ."','". $_POST['ItemTable_xqty_'.$v] ."','". $date ."',".$cost.",'0','". $_POST['xwh'] ."','". Session::get('sbranch') ."','". Session::get('sbranch') ."','0','0','0','','None','None','".Session::get('suser')."','".$itemdetail[0]['xunitsale']."','',".$year.",".$month.",'".$itemdetail[0]['xtypestk']."','0'),";
							...
							$vals .= "(" . Session::get('sbizid') . ",'". $keyfieldval ."','". $rownum ."','". $_POST['ItemTable_xitemcode_'.$v] ."','". $_POST['ItemTable_xitemcode_'.$v] ."','". $_POST['ItemTable_xqty_'.$v] ."','". $date ."',".$cost.",'0','". $_POST['xwh'] ."','". Session::get('sbranch') ."','". Session::get('sbranch') ."','0','0','0','','None','None','".Session::get('suser')."','".$itemdetail[0]['xunitsale']."','',".$year.",".$month.",'".$itemdetail[0]['xtypestk']."','0'),";
		...
		function confirmpost($sonum,$disc){
			$voucher = "";	
	...
	function cancelpost($sonum,$disc){
		$sta = "";
		//echo "rttt";
		//$org = $this->model->getCustomer($cus)[0]['xorg'];
		//$sonum = $_POST['xsonum'];

...
class Supayapproval extends Controller{
	...
	function cancel($reqnum){
		...
		if(!empty($req)){
			...
			$this->model->confirmConcat($updateCode, $where);
			$flag="Supplier Payment No : ".$reqnum." - Canceled Done";
		}
		$this->index($flag);
	}		
	...
	function renderTable($branch=""){
		...
		//print_r($row); die;
		return $this->createTable($fields, $row, "xtxnnum", URL."supayapproval/showreq/", URL."supayapproval/approve/", URL."supayapproval/cancel/");
	}

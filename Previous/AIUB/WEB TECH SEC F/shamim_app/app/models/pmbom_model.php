...
class Pmbom_Model extends Model{
	...
	public function edit($data,$dt,$bomcode,$row){
		...
		//print_r($header); print_r($detail);die;
			$where = "xrecflag='Live' and bizid = ". Session::get('sbizid') ." and xbomcode = '". $bomcode."'";
			$this->db->update('pmbommst', $header, $where);
			$where = "bizid = ". Session::get('sbizid') ." and xitemtype='Raw Material' and xbomcode = '". $bomcode."' and xrow=".$row;
	...
	public function getSingleBomDt($txnnum, $row){
		...
		//print_r($this->db->select("pabuziness", $fields));die;
		$where = " bizid = ". Session::get('sbizid') ." and xitemtype='Raw Material' and xbomcode = '". $txnnum ."' and xrow=".$row;
	...
	public function getbomdt($txnnum){
		$fields = array("xrow",
		"xrawitem","(select xdesc from seitem where pmbomdet.bizid=seitem.bizid and pmbomdet.xrawitem=seitem.xitemcode) as xrawdesc",	
		"xqty");
		//print_r($this->db->select("pabuziness", $fields));die;
		$where = " bizid = ". Session::get('sbizid') ." and xitemtype='Raw Material' and xbomcode = '". $txnnum ."'";
		//print_r($this->db->select("podet", $fields, $where, " order by xrow"));die;	
		return $this->db->select("pmbomdet", $fields, $where, " order by xrow");
	}

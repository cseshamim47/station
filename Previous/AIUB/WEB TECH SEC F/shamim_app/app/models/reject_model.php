...
class Reject_Model extends Model{
	...
	public function getSOStatus($sonum){
		...
		return $this->db->select("rejectmst", $fields, $where);
	}
	...
	public function create($mcols, $mvals, $cols, $vals){
		$mresults = $this->db->insertMultipleDetail('rejectmst',$mcols,$mvals," ON DUPLICATE KEY UPDATE xrcvamt = VALUES(xrcvamt),xcus = VALUES(xcus),xwh = VALUES(xwh),xquotnum = VALUES(xquotnum),xgrossdisc = VALUES(xgrossdisc),xrcvamt = VALUES(xrcvamt),xnotes = VALUES(xnotes),xyear = VALUES(xyear),xper = VALUES(xper),xpackcharge = VALUES(xpackcharge),xtranscost = VALUES(xtranscost)");
		$results = $this->db->insertMultipleDetail('rejectdet',$cols,$vals," ON DUPLICATE KEY UPDATE xitemcode = VALUES(xitemcode),xcur = VALUES(xcur), xrate = VALUES(xrate), xqty = VALUES(xqty), xwh = VALUES(xwh), xdisc = VALUES(xdisc), xexch = VALUES(xexch), xcur = VALUES(xcur), xcus = VALUES(xcus), xyear = VALUES(xyear), xper = VALUES(xper), xdispct = VALUES(xdispct)");
		
		return $mresults;	
	}
	public function recdelete($where){
		$this->db->dbdelete('rejectdet', $where);
	}
	public function getSales($sonum){
		$fields = array("*","(select xorg from secus where secus.bizid=rejectmst.bizid and secus.xcus=rejectmst.xcus) as xorg",
		"(select xadd1 from secus where secus.bizid=rejectmst.bizid and secus.xcus=rejectmst.xcus) as xadd1",
		"(select xphone from secus where secus.bizid=rejectmst.bizid and secus.xcus=rejectmst.xcus) as xphone");
		$where = "xrecflag='Live' and bizid = ". Session::get('sbizid') ." and xsonum = '". $sonum ."'" ;	
		return $this->db->select("rejectmst", $fields, $where);
	}
	...
	public function confirm($postdata,$where){
		...
		$this->db->update('rejectmst', $postdata, $where);
	...
	public function getsomilestone($txnnum){
		...
		//print_r($this->db->select("rejectmst", $fields, $where));die;
		return $this->db->select("vsomilestone", $fields, $where);
	}	
	public function getSalesList($status){
		$fields = array("date_format(xdate, '%d-%m-%Y') as xdate","xsonum","xcus","(select xorg from secus where secus.bizid=rejectmst.bizid and secus.xcus=rejectmst.xcus) as xorg","xcusdoc");
		//print_r($this->db->select("pabuziness", $fields));die;

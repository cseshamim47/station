...
class Posentry_Model extends Model{
	...
	public function getSingleRecord($imsenum, $xrow){
		$fields = array("ximsenum","xcus","xcusdt","xdate","xrow", "xitemcode as xitemcodeosp","xsup","xsupdt","xrdin","xuom","xdisc", "xsl","xdesc","xqty","xstdprice as xsalesprice","xpoint");
		//print_r($this->db->select("pabuziness", $fields));die;
		$where = "xrecflag='Live' and bizid = ". Session::get('sbizid') ." and ximsenum = '". $imsenum."' and xrow = '". $xrow ."' and xtxntype='Sales' and zemail='". Session::get('suser') ."'";	
	...
	public function posInvoicePickTable($branch,$limit=""){
		...
		//print_r($this->db->select("pabuziness", $fields));die;
		$where = " xrecflag='Live' and bizid = ". Session::get('sbizid') ." and xtxntype='Sales' and xbranch = '". $branch ."' and zemail='". Session::get('suser') ."' and xtxntype='Sales'";	
		return $this->db->selectDistinct("imsetxn", $fields, $where, " order by ximsenum", $limit);
	}
	
	public function getSalesByInv($imsenum){
		$fields = array("ximsenum","xrow", "xdesc","xqty","xsalesprice","xdisc","xpoint","xtotprice as xsalestotal");
		//print_r($this->db->select("pabuziness", $fields));die;
		$where = " xrecflag='Live' and bizid = ". Session::get('sbizid') ." and ximsenum = '". $imsenum."' and xtxntype='Sales' and zemail='". Session::get('suser') ."'";	
	...
	public function getUnconfirmedList($status, $branch){
		$fields = array("ximsenum", "xdate", "xcus", "xcusdt");
		//print_r($this->db->select("pabuziness", $fields));die;
		$where = " xrecflag='Live' and bizid = ". Session::get('sbizid') ." and xtxntype='Sales' and xstatus = '". $status ."' and xbranch = '". $branch ."' and zemail='". Session::get('suser') ."'";	
